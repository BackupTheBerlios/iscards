...
public class CreaArbol {
	...
	private void fa(Element r, Vector opciones) {
		...
		for (int i = 1; i < lista.getLength(); i = i + 2) {
			...
			if (tipoOpcion.equals("opcionSimple")) {
				// una OpcionSimple solo tiene como hijos la descripcion
				 ...
				 */
				NodeList listaHijos = nuevaRaiz.getChildNodes();
				// obtengo los hijos de la nueva raiz
				...
				// obtenemos la descripcion de la raiz
				String descripcion = ((Text) elementoDescripcion.getFirstChild()).getData();
				Vector opcionesNuevaOpcion = new Vector();
				// creamos un nuevo vector para la nueva OpcionCompuesta
	...
	private OpcionCompuesta formaArbol(Document documentoXML) {
		...
		r = documentoXML.getDocumentElement();
		// extraigo la raiz del documento
		// formo la Opcion que es la raiz de todo el arbol. Siempre es una opcion compuesta
		String nombre = r.getAttribute("nombre");
		// extraemos el atributo nombre de la raiz
		NodeList listaHijos = r.getChildNodes();
		...
		fa(r, opciones);
		return ((new OpcionCompuesta(opciones, nombre, descripcion)));
		//devolvemos la Opcion raiz del arbol
	}
