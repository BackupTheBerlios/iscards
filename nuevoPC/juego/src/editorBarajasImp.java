...
/**
 * <p>Título: GENESIS</p>
 * <p>Descripción: Implementación del interfaz gráfico del Editor de barajas del jugador</p>
...
public class editorBarajasImp extends editorBarajasGUI{
  ...
  /**
   * tabla Hash con todas las cartas disponibles por el usuario para personalizar una baraja .BAR
   */
  ...
  /**
   * Función para controlar el evento de cambiar el comboBox con los nombres de las cartas
   * y mostrar dicha carta, para facilitar la elección
   * @param e
   */
  ...
  /**
   * Función para controlar el evento del mouse en la lista de cartas disponibles
   * @param e
   */
  ...
  /**
   * Función para controlar el evento del mouse en la lista de cartas seleccionadas
   * @param e
   */
  ...
  /**
   * Función que carga la colección de cartas del usuario del archivo "usuario.rep"
   */
  private void cargarColeccionUsuario(){//añadir las cartas disponibles para la baraja
    try {
  ...
  /**
   * Función que carga la baraja personalizada del usuario del archivo "baraja.bar"
   * @param fichBaraja archivo con la baraja personalizada para cargar
   */
  private void cargarBarajaUsuario(String fichBaraja){//añadir la coleccion
    try {
          ...
          if (nombreUsuario.equals(usuario.getNombreUsuario())) {
            ...
            while (numeroDeBytesALeer >= 0) { // i==-1 es fin de fichero
              ...
              if (tablaCartasDisponibles.containsKey(nombre)){
                int repDisponibles = ((Integer)tablaCartasDisponibles.
                                      get(nombre)).intValue();
...
System.out.println(tablaCartasDisponibles.get("Luna llena"));
  ...
  /**
   * Función que añade la baraja guardada a la lista de las barajas del usuario
   */
  ...
  void jMenuFileGuardarComo_actionPerformed(ActionEvent e) {
    ...
    if(numCartas >= 40){
      ...
      JOptionPane.showMessageDialog(new JOptionPane(),
                                    "Debes tener un mínimo de 40 cartas en la baraja",
                                    "AVISO", JOptionPane.INFORMATION_MESSAGE);
  }
